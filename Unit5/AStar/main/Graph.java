...
public class Graph {
    ...
    public LinkedList<String> routeFirstPath(String fromLabel, String toLabel) {
        ...
        if(fromNode == null || toNode == null){
            throw new RuntimeException("routefirstpath, one of your dumb states is null");
        }
        ...
        while(!toExplore.isEmpty()){
            ...
            for(Node neighbor : removed.getNeighbors()){
                if(neighbor.getState() == null){
                    neighbor.setState(removed);
                    neighbor.setDistanceSoFar(removed.getDistanceSoFar() + removed.getPoint().distance(neighbor.getPoint()));
        ...
        Node curNode = toNode; 
        route.add(curNode.getLabel());
        while(curNode != fromNode){
            // Add the label of the point in the node to the result list
            curNode = curNode.getState();
            route.addFirst(curNode.getLabel());
        }    
        // end loop
    ...
    public LinkedList<String> routeDijkstra(String fromLabel, String toLabel) {
        ...
        Node toNode = _nodes.get(toLabel);
        if (fromNode == null || toNode == null) {
            throw new RuntimeException("routedijkstra, one of your dumb states is null");
        }
        ...
        while (!toExplore.isEmpty()) {
            ...
            for (Node neighbor : removed.getNeighbors()) {
                if (neighbor.getState() == null) {
                    neighbor.setState(removed);
                    neighbor.setDistanceSoFar(
                            removed.getDistanceSoFar() + removed.getPoint().distance(neighbor.getPoint()));
                    toExplore.add(neighbor);
                    _queueAddCount++;
                }
            }
        ...
        // Otherwise, a route was found. Each node, starting from toNode points to the
        LinkedList<String> route = new LinkedList<String>();
        Node curNode = toNode;
        route.add(curNode.getLabel());
        while (curNode != fromNode) {
