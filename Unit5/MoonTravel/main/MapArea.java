...
public class MapArea implements Comparable<MapArea> {
    ...
    public double distance(MapArea other) {
        return Math.sqrt(Math.pow(_row - other._row, 2) + Math.pow(_col - other._col, 2));
    }
    ...
    public boolean newDijkstraNode(MapArea previous, double newDistance) {
        if (_previous == null || newDistance < _fCost) {
            _fCost = newDistance;
            _previous = previous;
            return true;
        }
    ...
    public boolean newAStarNode(MapArea previous, double newDistance) {
        if (_previous == null || newDistance < _fCost) {
            _fCost = newDistance;
            _previous = previous;
            return true;
