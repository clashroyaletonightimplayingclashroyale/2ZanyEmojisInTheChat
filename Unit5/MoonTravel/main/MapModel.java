...
public class MapModel {
    ...
    public boolean routeDijkstra() {
        ...
        PriorityQueue<MapArea> queue = new PriorityQueue<MapArea>(new Comparator<MapArea>() {
            @Override
            public int compare(MapArea a1, MapArea a2) {
                return Double.compare(a1.getDistanceSoFar(), a2.getDistanceSoFar());
            }
        });
        ...
        while(!queue.isEmpty()) {
            ...
            for(MapArea neighbor : getNeighbors(crt)) {
                double newDistance = crt.getDistanceSoFar() + crt.distance(neighbor);
                if (neighbor.newDijkstraNode(crt, newDistance)) {
                    queue.add(neighbor);
                }
            }
        }
    ...
    public boolean routeAStar() {
        ...
        PriorityQueue<MapArea> queue = new PriorityQueue<MapArea>(new Comparator<MapArea>() {
            @Override
            public int compare(MapArea a1, MapArea a2) {
                double f1 = a1.getDistanceSoFar() + a1.distance(_targetArea);
                double f2 = a2.getDistanceSoFar() + a2.distance(_targetArea);
        ...
        while(!queue.isEmpty()) {
            ...
            for(MapArea neighbor : getNeighbors(crt)) {
                double newDistance = crt.getDistanceSoFar() + crt.distance(neighbor);
                if (neighbor.newAStarNode(crt, newDistance)) {
                    queue.add(neighbor);
                }
            }
        }
